...
const REGULAR_SYMBOLS_IDS = [...LOW_VALUE_IDS, ...MIDDLE_VALUE_IDS, ...HIGH_VALUE_IDS]
...
class SlotMachine {
	...
	async roll({
		...
		for(let x = 0; x < reels.length; x++ ) {
			...
			if (randomValue < scatterProbability[x]) {
				randomValue = await random()
				reel[Math.trunc(randomValue * REELS_HEIGHT)] = SCATTER_SYMBOL_ID
			}
			// ...SCATTER
			// ...SPECIAL SUBSTITUTE
		}
		...
		do {
			...
			if (scattersCount >= 3) {
				...
				if (!isFreeSpinsMode) {
					...
					const randomNumber = await random()
					specialSymbolId = REGULAR_SYMBOLS_IDS[Math.trunc(REGULAR_SYMBOLS_IDS.length * randomNumber)]
	...
	async generateRiskGameDescriptor({
		option = 0,
		pendingPayout,
	}) {
		const randomNumber = await random()
		let isWin = randomNumber > 0.5


		// WIN...
		if (isWin) {
			const payout = pendingPayout * 2
