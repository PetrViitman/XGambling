...
const REELS_LENGTHS = [3, 4, 5, 5, 4, 3]
const LOW_VALUE_IDS = [
	1, // ♣️ CLUBS
	2, // ♥️ HEARTS
	3, // ♦️ DIAMONDS
...
class SlotMachine {
	...
	async cascadeReels(reels, corruptionMap, isFreeSpinsMode) {
		// MAPPING CASCADE...
		...
		// PATCH...
		const reelsPatch = await this.getReelsPatch(reels, isFreeSpinsMode)
		for (let x = 0; x < REELS_COUNT; x++)
	...
	async getCascadeSteps({
		...
		do {
			...
			step.reelsPatch = await this.cascadeReels(reels, winMap, isFreeSpinsMode)
	...
	async generateSingleGameDescriptor({
		...
		do {
			...
			const subSteps = await this.getCascadeSteps({
