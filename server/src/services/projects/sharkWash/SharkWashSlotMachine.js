...
class SlotMachine {
	...
	async roll() {
		...
		for(let x = 0; x < reels.length; x++) {
			...
			const offsetY = Math.trunc(randomNumber * CELLS_PER_REEL_COUNT)
		...
		do {
			...
			if(
				...
				for(let i = 0; i < 3; i++) {
					...
					let reelIndex = Math.trunc(randomNumber * REELS_COUNT)
					...
					const cellIndex = Math.trunc(randomNumber * CELLS_PER_REEL_COUNT)
					reels[reelIndex][cellIndex] = SCATTER_SYMBOL_ID
				}
			}
			// ...PURCHASED BONUS PATCH
			
			const subSteps = await this.getCascadeSteps({reels, multiplierCapacity, bet})
	...
	async getCascadeSteps({
		...
		do {
			...
			step.reelsPatch = await this.cascadeReels(reels, winMap)
		} while (winDescriptor)
