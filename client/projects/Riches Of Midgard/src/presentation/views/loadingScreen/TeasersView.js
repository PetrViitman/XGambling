		...
		this.teasersViews = [
			{
				...
				texture: resources.teaser_left,
				offsetX: 55,
			},
			{
				text: dictionary.wild_multipliers_teaser_bmp,
				texture: resources.teaser_right,
				offsetX: 10,
			},
		].map(({text, texture, offsetX = 0}) => {
			const container = this.addChild(new Container)
			container.addChild(new Sprite(texture)).x = offsetX
