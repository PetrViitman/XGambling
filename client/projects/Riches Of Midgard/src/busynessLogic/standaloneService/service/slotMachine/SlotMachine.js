...
export const EMPTY_SYMBOL_ID = 0
...
export const HOT_SYMBOLS_IDS = [EMPTY_SYMBOL_ID, ...HIGH_VALUE_IDS]
...
export const SYMBOLS_IDS = [0, ...REGULAR_SYMBOLS_IDS, ...SPECIAL_SYMBOLS_IDS]
	...
	roll({
		...
		reels.forEach((reel, x) => {
			...
			if (!isLocked && Math.random() < scatterProbability[x])
				reel[Math.trunc(Math.random() * regularReelSize)] = SCATTER_SYMBOL_ID
			// ...SCATTER
			...
			if (!isLocked && Math.random() < wildProbability)
				reel[Math.trunc(Math.random() * regularReelSize)] = WILD_SYMBOL_ID
			// ...WILD
			// ...SPECIAL SUBSTITUTE
		})
		...
		do {
			...
			if (
				...
				for(let i = 1; i <= 3; i++) {
					...
					const cellIndex = Math.trunc(Math.random() * MINIMAL_REGULAR_REEL_LENGTH)
