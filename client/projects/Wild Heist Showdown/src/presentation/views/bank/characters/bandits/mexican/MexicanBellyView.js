...
export class MexicanBellyView extends BaseBellyView {
    initFront(assets) {
    ...
    setFlip(flipProgress) {
        ...
        this.gapsViews.forEach((view, i) => {
            ...
            view.x =  -110 * sin
            view.y =  70 - 20 * Math.abs(sin)
            view.scale.x = Math.sin(Math.PI * shiftedProgress)
            view.skew.x = -sin * 0.25
            view.alpha = Math.min(1, (1 - Math.min(1, Math.abs(view.x) / 110)) * 3 )
        })
    }
