			...
			.addAnimation({
				...
				onProgress: (progress) => {
					...
					this.cellsViews.forEach((view) => {
						...
						view.setBlur(progress) // блюр во время падения
			...
			.addAnimation({
				...
				onProgress: (progress) => {
					...
					if(
						...
						// removing blur
						cellsViews.forEach(view => { if (progress === 0) { 
							view.setUnblur(progress); } 
						else { 
						view.setBlur(progress); } });
					
					//было cellsViews.forEach(view => view.setBlur(1 - progress))
					}
					// ...FINAL DESCENDENCE
				...
				onFinish: () => {
					...
					for (let i = cellsViews.length - 2; i > 0; i--) {
						...
						cellsViews[i].setBlur(cellsViews[i - 1].blurProgress)
						cellsViews[i].scale.set(cellsViews[i - 1].scale.x)
					}
					// ...SYMBOLS SWAP

					// NEXT TOP SYMBOLS...
					const nextSymbolId = this.targetSymbolsIds?.pop() ?? getRandomSymbolId(index)
					cellsViews[0].presentSymbol(nextSymbolId)
					cellsViews[0].setBlur(1)
					// ...NEXT TOP SYMBOLS
		...
		await this
			...
			.addAnimation({
				...
				onProgress: (progress) => {
					...
					this.cellsContainer.pivot.y = subProgress * CELL_HEIGHT * 0.2
					this.cellsViews.forEach(view => {
						//view.setUnblur(progress)
						view.setSymbolFall(progress)
