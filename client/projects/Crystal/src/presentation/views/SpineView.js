...
export class SpineView extends Spine {
    ...
    mixAnimations({
        ...
        if (!this.animationsNames.includes(animationName1)
			|| !this.animationsNames.includes(animationName2)) {return this;}
    ...
    async playAnimation({
        ...
        if (this.currentAnimationName === name && this.isLoopMode) {
            return;
        }
    ...
    setTimeScale({
        ...
        if (!ignoreInversion) {inversionMultiplier = isReversed ? -1 : 1;}
    ...
    isPlaying() {
        ...
        return entry.trackTime < entry.animationEnd;
    }
