...
export class TextField extends Container {
	...
	lineBreakTriggerSymbol = ' '
	specialLineBreakSymbol = '║'
	breakLinesOnlyOnSpaces = true
	...
	constructor({
		...
		this.lineBreakTriggerSymbol = lineBreakTriggerSymbol
	...
	highlightArea(color = this.areaColor, alpha) {
		...
		this.areaGraphics.alpha = alpha ?? this.areaGraphics.alpha
	...
	setText(text = '', forceReset = false) {
		...
		const {charactersPerLineCount, lineBreakTriggerSymbol} = this
		...
		if(charactersPerLineCount) {
			...
			for (let i = 0; i < finalText.length; i++) {
				...
				if(
					isLineBreakExpected
					&& (!lineBreakTriggerSymbol || character === lineBreakTriggerSymbol)
				) {
					this.text += '\n'

					if(!lineBreakTriggerSymbol) {
						this.text += character
					}
					...
					if(character === this.specialLineBreakSymbol) {
						this.text += '\n'
						elapsedCharactersCount = 0
						isLineBreakExpected = false
					} else {
	...
	setSpecialLineBreakSymbol(symbol) {
		this.specialLineBreakSymbol = symbol
